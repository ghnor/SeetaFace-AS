...
void Net::Execute() {
	...
	LOGD("Net::Execute(),1 CheckInput()");

  // 1. check input blobs
  CheckInput();
  // 2. execute

  LOGD("Net::Execute(),nets_.size()=%d", nets_.size());

  for (int i = 0; i < nets_.size(); ++ i) {
	  LOGD("Net::Execute(), i=%d", i);

    nets_[i]->Execute();
  }
  LOGD("Net::Execute(), CheckOutput();");
...
void Net::CheckInput() {
  for (int i = 0; i < input_blobs_.size(); ++ i) {
    if (input_blobs_[i].data() == nullptr) {
      LOGD( "Net::CheckInput(), Net input haven't been initialized completely!");
      exit(0);
    }
  }
}
