...
//#include "D:\android\android-ndk-r10\toolchains\x86-4.8\prebuilt\windows-x86_64\lib\gcc\i686-linux-android\4.8\include\emmintrin.h"
...
class SURFFeatureMap : public FeatureMap {
 public:
  ...
  inline virtual void SetROI(const seeta::Rect & roi) {
    roi_ = roi;
    if (buf_valid_reset_) {
      memset(buf_valid_.data(), 0, buf_valid_.size() * sizeof(int32_t));
